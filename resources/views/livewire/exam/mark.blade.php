...
<x-card title="Mark Entry" separator progress-indicator>
        ...
        <x-form wire:submit.prevent="save">
            ...
            @foreach ($subjects as $key => $subject)
                <div class="flex justify-center item-center w-full gap-2">
                    ...
                    <x-button wire:confirm="Are you sure to delete this subject?" icon="o-trash"
                        wire:click="deleteSubject({{ $key }})" class="btn-xs btn-error " />
                    <x-button wire:confirm="Are you sure to delete this subject?" icon="o-trash"
                        wire:click="deleteSubject({{ $key }})" class="btn-xs btn-error " />
    ...
    <x-table :headers='[
        ...
        @scope('actions', $mark, $resultId)
            <div class="flex">
                <x-button icon="o-pencil-square" @click="$wire.modalEditOpen({{ $mark->student_id }})" spinner
                    class="btn-xs btn-primary text-white" />

                <a href="{{ route('print.result_sheet', [$resultId, $mark->student_id]) }}"
